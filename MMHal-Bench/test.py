...
if __name__ == '__main__':
    ...
    for i, record in enumerate(records):
        image_content = ', '.join(record['image_src'])
    ...
    hallucination_flags = [result["hallucination"] for result in evaluation_results]

    # Output results
    print('Average score: {:.2f}'.format(sum(scores) / len(scores)))
    print('Hallucination rate: {:.2f}'.format(sum(hallucination_flags) / len(hallucination_flags)))
